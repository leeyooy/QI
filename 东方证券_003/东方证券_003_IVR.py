...
stockClass = stockclass('data/stock', preRead=['DailyReturnSimple'])
...
# 获取每月月末的证券池
stockPools = stockClass.get_stockPools(EOMs, minDate=122)
...
for month in months:
    ...
    returnMonth = stockClass.get_data(secIDs=stockPool, dates=DUMs, fields=[
                                      'DailyReturnSimple'], showProcess=False)
    ...
    for secID in stockPool:
        # 获取该证券在该月的行情序列
        returnSec = returnMonth.loc[secID, :].sort_values(
            by='dates').set_index('dates')
        ...
        if not dataTotal.empty:
            X, y = sm.add_constant(
                dataTotal[['MKT', 'SMB', 'HML']]), dataTotal['Return']
            model = sm.OLS(y, X).fit()
            factorMonth[secID] = 1 - model.rsquared
        else:
...
# 存储计算结果
factorResult.to_csv('factor/final/东方证券_003_IVR.csv')
