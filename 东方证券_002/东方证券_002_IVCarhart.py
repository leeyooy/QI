...
# 获取前期计算好的MOM数据
dataMOM = pd.read_csv('factor/middle/东方证券_002_MOM.csv')
dataMOM.columns = ['dates', 'MOM']
dataMOM['dates'] = dataMOM['dates'].apply(lambda x: str(x))
dataMOM = dataMOM.set_index('dates')['MOM']
...
for month in months:
    ...
    for secID in stockPool:
        ...
        dataTotal = pd.DataFrame({'Return': returnSec['DailyReturnSimple'],
                                  ...
                                  'HML': dataHML.loc[BOMs[month]: EOMs[month]],
                                  'MOM': dataMOM.loc[BOMs[month]: EOMs[month]]}).dropna()
        # 如果当月有数据可以参与运算
        if not dataTotal.empty:
            X, y = sm.add_constant(dataTotal[['MKT', 'SMB', 'HML', 'MOM']]), dataTotal['Return']
            fit = sm.OLS(y, X).fit()
...
# 存储计算结果
factorResult.to_csv('factor/final/东方证券_002_IVCarhart.csv')
