...
for month in months:
    ...
    for secID in stockPool:
        ...
        dataTotal = pd.DataFrame({'dailyReturn': returnSec['DailyReturnSimple'],
                                  'MKT': dataMKT.loc[BOMs[month]: EOMs[month]]}).dropna()
        # 如果当月有数据可以参与运算
        if not dataTotal.empty:
            X, y = sm.add_constant(dataTotal['MKT']), dataTotal['dailyReturn']
            fit = sm.OLS(y, X).fit()
...
# 存储计算结果
factorResult.to_csv('factor/final/东方证券_002_IVCAPM.csv')
